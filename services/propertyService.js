...
class PropertyService {
  ...
  async filterPropertiesSharingFlat(filterRequest, page, size) {
    const { userId, propertyType, location, priceMin, priceMax, gender } = filterRequest;
  ...
  const {
    userId, type, location, priceMin, priceMax,search,
    gender, amenities, roomType, status, listedOn
  } = filterRequest;
  ...
  if (userId) {
    ...
    if (!user || user.isPremium === 0) {
      return { properties: [], currentPage: page, totalItems: 0, totalPages: 0 };
    }
  }
  ...
  if (amenities?.length) {
    query.$and = amenities.map(a => ({ amenities: { $regex: new RegExp(`\\b${a}\\b`, 'i') } }));
  }
  ...
  switch ((status || '').toLowerCase()) {
    ...
    case 'today':
      ...
      query.listedDate = { $gte: new Date(startToday), $lte: new Date(endToday) };
      break;
    case 'yesterday':
      const yesterday = new Date(Date.now() - 86400000);
      const startYest = new Date(yesterday.setHours(0, 0, 0, 0));
      const endYest = new Date(yesterday.setHours(23, 59, 59, 999));
      query.listedDate = { $gte: new Date(startYest), $lte: new Date(endYest) };
      break;
  }
  ...
  let properties = await PropertyDetails.find(query).sort({ createdOn: -1 }).skip(skip).limit(size).lean();
      ...
      .map(p => {
        ...
        return {
          ...
          number: contactedSet.has(id)
            ? (isSpecificUser ? '9' + Math.floor(100000000 + Math.random() * 900000000) : p.number)
            : '0',
  ...
  try {
    ...
    // Today counts
    counts.todayResidentialRental = await countProperties("Residential Rent", { start: startOfToday, end: endOfToday });
    counts.todayResidentialSell = await countProperties("Residential Sell", { start: startOfToday, end: endOfToday });
    counts.todayCommercialRent = await countProperties("Commercial Rent", { start: startOfToday, end: endOfToday });
    counts.todayCommercialSell = await countProperties("Commercial Sell", { start: startOfToday, end: endOfToday });

    // Active counts
    counts.activeResidentialRental = await countProperties("Residential Rent");
...
number: contactedSet.has(id)
  ? (isSpecificUser
      ...
      : String(p.number))
  : '0',          name: contactedSet.has(id) ? p.name : '0'
        };
      })
      .filter(p => !excludedStatuses.includes(p.status));
  }
